...
**Location**: `/apps/web`（未作成）  
**Purpose**: Next.js による UI。ドメイン（auth/receipts/ledger/knowledge 等）ごとに feature-first でコンポーネント・hooks・型をまとめる。  
**Example**: `/apps/web/src/features/receipts/{components,hooks,services}` のようにドメイン配下で自己完結。

### API Service (planned)
**Location**: `/services/api`（未作成）  
**Purpose**: Go + Echo。ハンドラは薄く、service 層に業務ロジック、repository 層で GCP リソース（Firestore/Storage 等）を抽象化。  
**Example**: `/services/api/internal/receipts/{handler,service,repository}` として依存方向を内向きに固定。

### Async Workers (planned)
**Location**: `/services/worker` など（未作成）  
**Purpose**: Pub/Sub/Cloud Tasks からの OCR ジョブを処理し、明細を更新する単機能ワーカー。  
**Example**: `/services/worker/internal/ocr` にジョブハンドラを集約。

### Infrastructure as Code
**Location**: `/infra/terraform`（未作成）  
**Purpose**: 環境別（dev/stg/prod）ワークスペースとモジュール分割（identity, firestore, storage, pubsub, tasks, monitoring, ci）。  
**Example**: `/infra/terraform/modules/storage` を複数環境から再利用し、remote state を統一管理。
